...
function processCSV(csvString) {
    ...
    const innerMargin = 100; // Margen interno para los puntos
        // Eje X
    ctx.beginPath();
    ctx.moveTo(0, canvas.height);  // Desde la esquina inferior izquierda
    ctx.lineTo(canvas.width, canvas.height);  // Hasta la esquina inferior derecha
    ctx.stroke();

    // Eje Y
    ctx.beginPath();
    ctx.moveTo(0, 0);  // Desde la esquina superior izquierda
    ctx.lineTo(0, canvas.height);  // Hasta la esquina inferior izquierda
    ctx.stroke();
    ...
    data.forEach(point => {
        const x =  innerMargin + ((point.X - xMin) / (xMax - xMin)) * (canvas.width - 2 * ( innerMargin));
        const y = canvas.height  - innerMargin - (((point.Y - yMin) / (yMax - yMin)) * (canvas.height - 2 * (  innerMargin)));
        ...
        ctx.fill();
        // Borde del punto
        ctx.strokeStyle = 'rgba(90, 34, 139, 0.5)'; // Color del borde del punto
